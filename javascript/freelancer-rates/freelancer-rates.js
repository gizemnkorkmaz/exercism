...
export function monthRate(ratePerHour, discount) {
  let monthsRate = dayRate(ratePerHour) * 22;
  let monthsDiscount = monthsRate * discount;
  return Math.ceil(monthsRate - monthsDiscount);
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  let dailyRate = Math.round(
    dayRate(ratePerHour) - dayRate(ratePerHour) * discount
  );
